...
def test_valid_stash_address():
    from stashlib import is_valid_stash_address

    main = valid_stash_address()
    test = valid_stash_address('testnet')

    assert is_valid_stash_address(main) is True
    assert is_valid_stash_address(main, 'mainnet') is True
    assert is_valid_stash_address(main, 'testnet') is False

    assert is_valid_stash_address(test) is False
    assert is_valid_stash_address(test, 'mainnet') is False
    assert is_valid_stash_address(test, 'testnet') is True


def test_invalid_stash_address():
    from stashlib import is_valid_stash_address

    main = invalid_stash_address()
    test = invalid_stash_address('testnet')

    assert is_valid_stash_address(main) is False
    assert is_valid_stash_address(main, 'mainnet') is False
    assert is_valid_stash_address(main, 'testnet') is False

    assert is_valid_stash_address(test) is False
    assert is_valid_stash_address(test, 'mainnet') is False
    assert is_valid_stash_address(test, 'testnet') is False
...
def test_parse_masternode_status_vin():
    from stashlib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % stashlib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(stashlib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(stashlib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(stashlib.blocks_to_seconds(16616)) == 2612035
