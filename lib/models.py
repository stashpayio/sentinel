...
class GovernanceObject(BaseModel):
    ...
    # sync stashd gobject list with our local relational DB backend
    @classmethod
    def sync(self, stashd):
        golist = stashd.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_stashd(stashd, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_stashd(self, stashd, rec):
        import decimal
        import stashlib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from stashd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from stashd, with every run
        ...
        # get/create, then sync payment amounts, etc. from stashd - Stashd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(stashd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from stashd! %s" % e)
            govobj.vote_delete(stashd)
            return (govobj, None)
    ...
    def vote_delete(self, stashd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(stashd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, stashd, signal, outcome):
        import stashlib
        ...
        output = stashd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = stashlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(stashd, signal)

    def sync_network_vote(self, stashd, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = stashd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = STASHD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 stash addr, non-multisig
            if not stashlib.is_valid_stash_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Stash address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # Stash Core restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (stashlib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = STASHD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not stashlib.is_valid_stash_address(addr, config.network):
    ...
    def hash(self):
        import stashlib
        return stashlib.hashit(self.serialise())
