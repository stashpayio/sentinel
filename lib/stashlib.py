...
def is_valid_stash_address(address, network='mainnet'):
    ...
    stash_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Stash address
        return False

    if (address_version != stash_version):
        return False
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('stash.conf')
