...
def has_stash_conf():
    ...
    valid_stash_conf = False

    # ensure stash_conf exists & readable
    #
    # if not, print a message stating that Stash Core must be installed and
    # configured, including JSONRPC access in stash.conf
    try:
        f = io.open(config.stash_conf)
        valid_stash_conf = True
    except IOError as e:
        print(e)

    return valid_stash_conf
...
def main():
    ...
    if not has_stash_conf():
        print("StashCore must be installed and configured, including JSONRPC access in stash.conf")
        sys.exit(1)
