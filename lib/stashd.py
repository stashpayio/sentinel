...
class StashDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some stashd methods
    ...
    @classmethod
    def from_stash_conf(self, stash_dot_conf):
        from stash_config import StashConfig
        config_text = StashConfig.slurp_config_file(stash_dot_conf)
        creds = StashConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = stashlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = stashlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = stashlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_stashd_proto_version_with_sentinel_ping)
